...
$_config = isset($config["github"][$url_array[0]]) ? $config["github"][$url_array[0]] : _die(10001);
//非目录中断
  ...
  function method_default() {
      ...
      */
      if (($GLOBALS["_config"]["CacheTime"] == 0 or time() - @filemtime($CacheFile) < $GLOBALS["_config"]["CacheTime"]) && file_exists($CacheFile)) {
        ...
        if ($Cache_json->webp) {
          $temp_path = $GLOBALS["_config"]["path"] .$url_info_file["dirname"] == "/" ? $url_info_file["dirname"] . $url_info_file["filename"] . ".webp" : $url_info_file["dirname"] . "/" . $url_info_file["filename"] . ".webp";
        } else {
          $temp_path =$GLOBALS["_config"]["path"] .$GLOBALS["real_url_p"];
        }
        //已缓存 立即导航jsdelivr
        //如果设置跳转缓存更加高效
        if (!$GLOBALS["_config"]["AnyAgency"])
        {
          header('Location: https://cdn.jsdelivr.net/gh/' . $GLOBALS["_config"]["username"] . "/" . $GLOBALS["_config"]["repos"] .$temp_path);
        }else{
          $data=file_get_contents('https://cdn.jsdelivr.net/gh/' . $GLOBALS["_config"]["username"] . "/" . $GLOBALS["_config"]["repos"] .$temp_path);
      ...
      if(!$GLOBALS["_config"]["InstallAgency"])
      {
        header('Location: ' . $GLOBALS["_config"]["site"] . $GLOBALS["_config"]["path"] . $GLOBALS["real_url_p"]);
      }else{
        $data=file_get_contents( $GLOBALS["_config"]["site"] . $GLOBALS["_config"]["path"] . $GLOBALS["real_url_p"]);
        foreach($http_response_header as $value){
          header($value);
        }
        echo $data;
      ...
      if ($if_webp) {
        ...
        if($temp_img !== false) {
          ...
          $temp_path = $GLOBALS["_config"]["path"] .$GLOBALS["real_url_n"];
