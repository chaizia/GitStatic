...
function user_info($username) {
    return json_decode(api_get("https://api.github.com/users/" . $username));
}
//不存在会返回一个message Not Found 获取用户基本信息
function repos_all($username) {
    return json_decode(api_get("https://api.github.com/users/" . $username . "/repos"));
}
//获取所有repos
function repos_info($username, $reposname) {
    return json_decode(api_get("https://api.github.com/repos/" . $username . "/" . $reposname));
}
//获取repos info
function repos_path($username, $reposname, $path) {
    return json_decode(api_get("https://api.github.com/repos/" . $username . "/" . $reposname . "/contents/" . $path));
}
//获取repos 目录内容
